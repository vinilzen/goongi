 ...
 * $trackback = new Trackback('vizitkiish', 'Ran Aroussi', 'UTF-8');
 * ?></code>
 * 
 * ==============================================================================
 * 
 * @version $Id: class_vizitki_trackback.php 1 2009-01-10 12:24:57Z john $
 * @copyright Copyright (c) 2004 Ran Aroussi (http://www.vizitkiish.org)
 * @author Ran Aroussi <ran@vizitkiish.org> 
...
class Trackback
{
  var $vizitki_name = '';    // Default vizitki name used throughout the class (ie. vizitkiish)
  ...
  function Trackback($vizitki_name, $author, $encoding = "UTF-8")
  {
    $this->vizitki_name = $vizitki_name;
    $this->author = $author;
    ...
    $this->bname    = ( !empty($_POST['vizitki_name']) ? $_POST['vizitki_name'] : ( !empty($_GET['vizitki_name'])  ? $_GET['vizitki_name']  : NULL ) );
   ...
   * include('trackback_cls.php');
   * $trackback = new Trackback('vizitkiish', 'Ran Aroussi', 'UTF-8');
   * if ($trackback->ping('http://tracked-vizitki.com', 'http://your-url.com', 'Your entry title')) {
  ...
  function ping($tb, $url, $title = "", $excerpt = "")
  {
    ...
    if (empty($excerpt)) {
        $excerpt = "I found your entry interesting do I've added a Trackback to it on my wevizitki :)";
    } 
    ...
    $tb_send = "url=" . rawurlencode($url) . "&title=" . rawurlencode($title) . "&vizitki_name=" . rawurlencode($this->vizitki_name) . "&excerpt=" . rawurlencode($excerpt); 
