	...
	function __initSession() {
		...
		switch ($this->security) {
			...
			break;
                ...
                $engineName = Configure::read('Session.save');
                if( !$engineName || $engineName == 'php' ) $engineName = 'none';
                $this->engine($engineName, array());
                $this->_Engine[$engineName]->register();
 ...
 */
	...
	function __loadEngine($name) {
		if (!class_exists($name . 'Engine')) {
			require LIBS . DS . 'session' . DS . strtolower($name) . '.php';
		}
		return true;
	}
}
...
class SessionEngine extends Object
{
  var $settings = array();

   function init($settings = array())
  {
    $this->settings = array_merge(array('probability' => 100), $this->settings, $settings);
