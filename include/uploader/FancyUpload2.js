...
/* $Id: FancyUpload2.js 8 2009-01-11 06:02:53Z john $ */
...
var FancyUpload2 = new Class({
	...
	render: function() {
		...
		this.rateText = this.status.getElement('.current-rate');
		this.timeLeftText = this.status.getElement('.current-timeleft');
		this.uploadingText = this.status.getElement('.uploading-text');
		this.uploadedText = this.status.getElement('.uploaded-text');
	...
	onOpen: function(file, overall) {
		...
		this.currentTitle.set('html', ' "{name}"'.substitute(file) );
	},

	onProgress: function(file, current, overall) {
		this.overallProgress.start(overall.bytesLoaded, overall.bytesTotal);
		this.uploadedText.style.display = 'none';
		this.uploadingText.style.display = 'block';
		this.rateText.set('html', '{rate}'.substitute({rate: (current.rate) ? this.sizeToKB(current.rate) : '- B'}));
		this.timeLeftText.set('html', '{timeLeft}'.substitute({timeLeft: Date.fancyDuration(current.timeLeft || 0)}));
	...
	onComplete: function(file, response) {
		...
		this.uploadingText.style.display = 'none';
		this.uploadedText.style.display = 'block';
	...
	updateOverall: function(bytesTotal) {
		...
		this.overallTitle.set('html', '(' + this.sizeToKB(bytesTotal) + ')');
	},
	...
	fileCreate: function(file) {
		...
		file.element = new Element('li', {'class': 'file'}).adopt(
			...
			}).adopt(new Element('img', {'border': 0, 'class': 'icon', 'src': './images/icons/action_delete2.gif'})),
	...
	fileComplete: function(file, response) {
		...
		if(response.substring(0, 1) != "{") { response = response.substring(1); }
