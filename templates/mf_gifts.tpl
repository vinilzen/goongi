...
<td style="text-align: right;vertical-align: middle;">
  ...
  <tr><td class='button' nowrap='nowrap'>
    <img src='./images/icons/gifts16.gif' border='0' class='icon'><a href="user_messages_new.php">{lang_print id=80000026}</a>
  </td></tr></table>
</td>
</tr>
</table>
...
<br>
  ...
  {if $maxpage > 1}
    <div align='center'>
      {if $p != 1}<a href='mf_gifts_user.php?user={$owner->user_info.user_username}&p={math equation='p-1' p=$p}'>&#171; {lang_print id=182}</a>{else}<font class='disabled'>&#171; {lang_print id=182}</font>{/if}
      {if $p_start == $p_end}
        &nbsp;|&nbsp; {lang_sprintf id=184 1=$p_start 2=$total_vars} &nbsp;|&nbsp; 
      {else}
        &nbsp;|&nbsp; {lang_sprintf id=185 1=$p_start 2=$p_end 3=$total_vars} &nbsp;|&nbsp; 
      {/if}
      {if $p != $maxpage}<a href='mf_gifts_user.php?user={$owner->user_info.user_username}&p={math equation='p+1' p=$p}'>{lang_print id=183} &#187;</a>{else}<font class='disabled'>{lang_print id=183} &#187;</font>{/if}
      </div>
    </div>
  {/if}
        {literal}
...
{if $total_vars == 0}
  ...
  <td class='result'><img src='./images/icons/bulb16.gif' border='0' class='icon'>{lang_print id=80000042}</td>
  </tr></table>
  </div>
...
{cycle name="startrow3" values="<tr align=center>,,,,"}
 
<td width="200" height="150"><div style='padding-bottom: 10px;' id='gift_{$con.gift_id}'><table><tr><td align="center">
    <a href="javascript:TB_show('{lang_print id=8000025} {$owner->user_displayname}', 'mf_gift_view.php?view={$con.gift_id}&user={$con.to}&TB_iframe=true&height=420&width=400', '', './images/trans.gif');"><img src="mf_gifts/{$con.file}_thumb.{$con.filetype}" border="0"></a><br>
