...
if($gift->user_gifts_total($ownergift) > 0){
	$plugin_vars[menu_profile_tab] = Array('file'=> 'profile_gifts_tab.tpl', 'title' => 80000100);//80000007);
}

if($ownergift != $user->user_info[user_id] and $user->user_exists != 0){
	$plugin_vars[menu_profile_menu] = Array('file' => "mf_gifts_send.php?to=".$owner->user_info[user_username]."", 'icon' => 'gifts16.gif', 'title' =>80000100);// 80000026);
}

$count_img = $database->database_num_rows($database->database_query("SELECT id FROM mf_gifts_data WHERE status=1"));
if($count_img > 0) {
	$plugin_vars[menu_user] = Array('file' => 'mf_gifts_user.php', 'icon' => 'gifts16.gif', 'title' => 80000100);//80000007);
}
