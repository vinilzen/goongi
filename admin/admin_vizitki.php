...
{
  $sql = "DELETE FROM se_vizitki_city WHERE vizitkisetting_id='{$vizitki_country_id}' LIMIT 1";
  $resource = $database->database_query($sql) or die($database->database_error()." <b>SQL was: </b>$sql");
...
else if( $task=="editcity" )
{
  // Get langvar id
//  $sql = "SELECT * FROM se_vizitki_country WHERE vizitkisetting_id 	='{$vizitkientrycat_id}' LIMIT 1";
  $sql = "UPDATE se_vizitki_city SET vizitkisetting_country='{$vizitkientrycat_title}' WHERE vizitkisetting_id='{$vizitkientrycat_id}' LIMIT 1";
  $resource = $database->database_query($sql) or die($database->database_error()." <b>SQL was: </b>$sql");
